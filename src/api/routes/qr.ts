...
router.post('/reassign', requireAuth, requireRole('guard'), async (req: Request, res: Response) => {
  try {
    ...
    if (existingPendingBatch) {
      ...
      const { error: pendingBatchError } = await supabase.from('payout_batches').insert({
        id: pendingBatchId,
      ...
      if (pendingBatchError) {
        ...
        // No PII is logged. A structured logger may replace this in a future phase per Ledger §13.6.
        console.error(
          '[qr] Error creating pending batch for QR replacement fee',
          pendingBatchError?.message
        );
    ...
    if (pendingBatchId) {
      const { error: feeItemError } = await supabase.from('payout_batch_items').insert({
    ...
    return res.status(200).json({
      message:
        'QR card reassigned successfully. Replacement fee (R10.00) will be deducted from your next payout.',
      old_qr: {
        id: currentQr.id,
